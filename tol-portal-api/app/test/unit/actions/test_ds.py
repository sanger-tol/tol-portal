...
class TestActionsWithDataSources:
    ...
    def test__404(
        ...
        """
        no action found by `action_id` -> raise 404 error
        """
    ...
    def test__200(
        ...
        - `sql_ds.upsert()` on user_action
        - `prefect_ds.upsert()` on flow_run
        """
        ...
        sql_ds.get_one.side_effect = self.__mock_get_one
        ...
        assert sql_ds.upsert.call_count == 1
        assert sql_ds.upsert.call_args[0][0] == 'user_action'
        mock_data_object_list = sql_ds.upsert.call_args[0][1]
    ...
    def __mock_get_one(
        ...
        if type_ == 'user':
            return self.__mock_user(id_)
        else:
            return self.__mock_action(id_)
